...
class MockStorageManager : public ITranscationalStorageManager {
 private:
  std::unordered_map<std::string, std::vector<SchemaItem>> db_schema_;
  std::unordered_map<std::string, std::vector<Tuple>> data_;

 public:
  MockStorageManager(const std::unordered_map<std::string, std::vector<SchemaItem>> &, const std::unordered_map<std::string, std::vector<Tuple>> &);
  ...
  auto insert_tuple(const std::string& rel, std::vector<std::unique_ptr<Value>>& arr) -> bool override;
  auto delete_tuple(const TupleId&) -> bool override;
};

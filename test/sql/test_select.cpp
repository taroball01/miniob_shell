...
bool check_stacks_empty(ParserContext& context) {
  return context.str_stack_.empty() 
      && context.attr_stack_.empty()
      && context.value_stack_.empty()
      && context.operand_stack_.empty()
      && context.pred_stack_.empty();
}
...
BOOST_AUTO_TEST_CASE(test_data_type) {
  ...
  std::string semi = ";";
  auto sw_cond = [&sql, &semi](const std::string& cond) -> std::string {
    return sql + cond + semi;
  };
  ...
  int result = sql_parse(sw_cond(cond), pc);
  {
    ...
    auto& lattr = dynamic_cast<const OperandAttr*>(&lchild)->get_attr();
  ...
  // -------------------------- data type: float
  {
    ...
    auto* query = pc.get_query<SelectQuery>();
    auto* leaf = dynamic_cast<PredicateLeaf*>(query->get_conditions().get());
    ...
    BOOST_CHECK_EQUAL(dynamic_cast<const OperandAttr*>(&rchild)->get_attr().get_attribute(), "credit");
  ...
  // -------------------------- data type
  {
    ...
    auto& lchild = leaf->get_lchild();
    BOOST_CHECK(lchild.get_operand_type() == OperandType::COT_ATTR);
    BOOST_CHECK_EQUAL(dynamic_cast<const OperandAttr*>(&lchild)->get_attr().get_attribute(), "workingyear");
    auto& rval = dynamic_cast<const OperandVal*>(&rchild)->get_value();
...
BOOST_AUTO_TEST_CASE(test_select_predicate_conjunctions_precedence) {
...

  {
    ...
    auto* and_pred = dynamic_cast<const PredicateAnd*>(&(not_pred->get_child()));
    auto& lchild = and_pred->get_lchild();
...

  {
    ...
    auto* not_pred = dynamic_cast<PredicateNot*>(&pred);
    BOOST_CHECK(not_pred != nullptr);
    auto* and_pred = dynamic_cast<const PredicateOr*>(&(not_pred->get_child()));
    auto& lchild = and_pred->get_lchild();
    BOOST_CHECK(lchild.get_node_type() == PredicateType::CNT_LEAF);
    auto& l_leaf = lchild.get_as<PredicateLeaf>();
    BOOST_CHECK(l_leaf.get_op() == CompareOp::OP_EQ);
